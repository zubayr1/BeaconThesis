...
fn listen_to_client(mut stream: TcpStream) {
    let msg : &[u8; 16]= b"Hello from node2";
...
fn handle_server(node2_port: u32) {
   // println!("server node2");
    let anycast = String::from("0.0.0.0:");

    let address = [anycast.to_string(), node2_port.to_string()].join("");

    let listener = TcpListener::bind(address).unwrap();
    // accept connections and process them, spawning a new thread for each one
    println!("Server node2 listening on port {}", node2_port);
...
fn match_tcp_client(address: String, node_port: u32)
{
    match TcpStream::connect(address) {
        Ok(mut stream) => {
           // println!("Successfully connected to server by node2 in port {}", node_port);

            let msg : &[u8; 16]= b"Hello from node2";

            stream.write(msg).unwrap();
            println!("Sent Hello from node2, awaiting reply...");
...
//init function
pub fn initiate_node2(random_number: u32, node_port_start: u32) {
